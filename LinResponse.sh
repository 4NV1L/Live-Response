...
checks() { # Checks script for the /tmp directory and root user 
...
echo -n > "$logfile"
log "##  Live Linux Incident Response data collection script ## "
...
socketinfo=`ss -aeirO 2>/dev/null`
if [ "$netrouting" ]; then
  log "[+] Collecting network open raw socket information" 
  log "ss -aeirO > $saveto/socketinfo.txt" 2>&1
  ss -aeirO > "$saveto/socketinfo.txt" 2>&1
else
    echo -e "\e[00;33m └─[ \e[0m \033[0;91m No information available for Command: ss -aeirO  \e[0m \e[00;33m ] \e[0m" 
fi
